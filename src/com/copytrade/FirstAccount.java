...
public class FirstAccount {
	...
	public void performBankNiftyTrade(TradeParams tradeParams, Integer lots) throws KiteException, IOException {
		try {
			...
			System.err.println("-------KiteException Error for First Account-------" +e.getLocalizedMessage());
			e.printStackTrace();
		} catch (IOException e) {
			System.err.println("-------IOException Error for First Account-------" +e.getLocalizedMessage());
			e.printStackTrace();
		}
	...
	private void handleBankNiftyOptions(String tradeSymbol, String orderType, Integer lots)
		...
		if (orderType.contains("sell")) { // Perform sell operation
			...
			try {
				...
				System.err.println("-------InputException Error for First Account -------" +e.message);
				e.printStackTrace();
			} catch (KiteException e) {
				System.err.println("-------KiteException Error for First Account -------" +e.message);
				e.printStackTrace();
			} catch (Exception e) {
				System.err.println("-------Exception Error for First Account -------" +e.getMessage());
				e.printStackTrace();
			}
